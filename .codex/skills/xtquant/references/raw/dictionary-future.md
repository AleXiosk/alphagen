...
- Official URL: https://dict.thinktrader.net/dictionary/future.html
...
获取该数据前，需要通过download_history_data接口下载数据，下载时period参数需指定为historymaincontract
...
| stockStatus | int | 证券状态，期货此数据不来自交易所，是根据行情所属时间段计算的值 |
...
1.该数据通过get_market_data和get_market_data_ex接口获取 2.获取数据前需要先用download_history_data下载历史数据 3.VIP 权限数据在新窗口打开
